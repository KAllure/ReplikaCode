...
from cakechat.utils.env import init_keras_horovod

init_keras_horovod(hvd)
...
def train(model_init_path=None,
          ...
          workers_num=HOROVOD_WORKERS_NUM,
    ...
    # check the number of required and available workers
    if hvd.size() != workers_num:
        raise AssertionError('\nNumber of horovod workers ({}) doesn\'t match the number of workers in config ({}).'
                             '\nCheck cakechat/config.py file and your multi-GPU setting.'.format(
                                 hvd.size(), workers_num))
