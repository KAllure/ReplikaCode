...
class CakeChatModel(AbstractKerasModel, WithLogger):
    def __init__(self,
        ...
        :param model_init_path: Path to a directory with model's saved weights for layers intialization
        ...
        self._rnn_class = CuDNNGRU if use_cudnn else GRU
    ...
    @property
    def model_path(self):
        return self._model_init_path or os.path.join(self._model_dir, self.model_id)
    ...
    def _utterance_encoder(self, tokens_emb_model):
        ...
        bidir_enc = Bidirectional(
            layer=self._rnn_class(units=self._params.hidden_layer_dim, return_sequences=True, name='encoder'),
