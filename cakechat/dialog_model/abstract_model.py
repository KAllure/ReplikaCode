...
class AbstractModel(WithLogger, metaclass=abc.ABCMeta):
    ...
    def __init__(self, model_resolver_factory=None, metrics_serializer=None):
        """
        :param model_resolver_factory: a factory of `luka_utils.files.resolver.AbstractFileResolver` that
            takes model path and returns a file resolver object
        :param metrics_serializer: an instance compatible to `luka_ml.metrics.utils.MetricsSerializer` interface
        :return:
        """
    ...
    def resolve_model(self):
        ...
        if not self.__model_resolver_factory(self.model_path).resolve():
        ...
        self._logger.info('Loaded model: {}'.format(self.model_path))
        self._logger.info('Model metrics: {}'.format(self._metrics))
