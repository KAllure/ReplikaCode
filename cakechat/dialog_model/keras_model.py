...
class AbstractKerasModel(AbstractModel, metaclass=abc.ABCMeta):
    ...
    def _to_keras_callbacks(callbacks):
        """
        Casts luka-ml-based callbacks (see `luka_ml.models.callbacks`) to the keras-based ones (instances of
            `keras.callbacks.Callback`)
        :param callbacks:
        :return:
        """
        keras_callbacks = []
        for luka_callback in callbacks:
            if isinstance(luka_callback, AbstractKerasModelCallback):
                keras_callback = _KerasCallbackAdapter(luka_callback)
            elif isinstance(luka_callback, ParametrizedCallback):
                keras_callback = luka_callback.callback
            else:
                raise ValueError('Unsupported callback type: {}'.format(type(luka_callback)))
